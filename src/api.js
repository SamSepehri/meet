...
export const getEvents = async () => {
    ...
    if (token) {
        removeQuery();
        const url = 'https://onjn9p7r1k.execute-api.eu-west-2.amazonaws.com/dev/api/get-events' + '/' + token;
...
const getToken = async (code) => {
    try {
        ...
        const response = await fetch('https://onjn9p7r1k.execute-api.eu-west-2.amazonaws.com/dev/api/token' + '/' + encodeCode);
        if (!response.ok) {
